...
#define SAMPLE_NUM 210 //采样点数

extern bit Stop_Aquire;                   //停止采样标志位
extern uint16 *BGV;                       //BGV电压值 mV

uint16 GetVoltage(uint8 chx, uint16 lsb); //获得chx通道电压值 mV
uint16 *GetADC(uint8 chx, uint8 scale_h); //获得chx通道ADC采样值
uint16 GetBGADC();                        //获得内部参考ADC
void GetBGV(void);						  //获得内部参考电压BGV mV
